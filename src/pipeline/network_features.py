...
def cal_tech_net(path, visualize_net: bool=False):
    ...
    if visualize_net:
        ...
        plt.savefig("../network_data/temp-visualization/"+f"{path.split('/')[-1]}-VISUALIZATION.png")
...
def extract_features(args_dict: dict):
    ...
    t_path = f"../network_data/{incubator}_{tech_type}/"
    s_path = f"../network_data/{incubator}_{social_type}/"
    proj_inc_path = params_dict["incubation-time"][incubator]
    outfile_path = f"../network_data/netdata/clean-{incubator}-network-data-{tech_num}-{social_num}.csv"
